...
fill! = |items, config, key, index, found|
	if List.is_empty(items) Ok(found) else {
		...
		context = Json.to_str_try(List.map(batch, |item| { sent_id: item.sent_id, sentence: item.sentence, word: item.word }))?
...
apply = |items, glosses, found| match (items, glosses) { ([], []) => Ok(found), ([item, .. as rest], [gloss, .. as more]) => apply(rest, more, List.append(found, { gloss: Str.trim(gloss), sent_id: item.sent_id, sentence: item.sentence, word: item.word })), _ => Err(GlossCountMismatch) }
