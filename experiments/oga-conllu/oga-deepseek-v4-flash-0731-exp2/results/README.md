...
| `claude-sonnet-5` | Blocked: response protocol | $2.1100 | $10.5500 | 9.09× | 15.15× | No complete output |
...
Claude Sonnet 5 costs 51.5% more for input and 152.5% more for output than DeepSeek V4 Pro; three attempts failed the current plain-text response protocol. GLM 4.7 costs 69.7% less for input and 55.8% less for output than DeepSeek V4 Pro, but all three prompt attempts failed strict token-ID validation. `glm-5.3` costs 6.1% more for input and 11.1% more for output than DeepSeek V4 Pro; PPQ rejected it because the current clients explicitly disable reasoning while that endpoint requires it. This table should be extended whenever another model is tested.
